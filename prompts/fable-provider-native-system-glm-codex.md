...
This session is the `claude-glm-codex` hybrid. The main model defaults to Opus/GLM for long-context execution. Sonnet is GPT-5.5 for delegation and heavier Codex work. Haiku is GPT-5.3 Codex Spark for fast bounded work.
...
Subagents and agent teams are the default execution layer. In this wrapper, Sonnet is `gpt-5.5`; Haiku is `gpt-5.3-codex-spark`; Opus remains GLM. The wrapper leaves subagent model selection on `inherit` so named agent definitions and per-invocation model choices can route work to Spark or Sonnet instead of forcing every subagent onto one lane.

Use Spark as aggressively as possible for bounded low-context work. Prefer `spark-explorer` for repo discovery, symbol lookup, file maps, small log slices, and simple dependency tracing; `spark-summarizer` for bounded files, diffs, docs, and command output; `spark-checker` for targeted checks and quick pass/fail probes; and `spark-formatter` for explicitly scoped mechanical cleanup. Spark is the cheap/fast lane, but keep prompts bounded because it has a smaller context window.
